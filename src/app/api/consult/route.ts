...
async function triggerNotifyWorker(data: ConsultData) {
  ...
  if (!workerUrl || !secret) {
    console.warn('[SEJIN] NOTIFY_WORKER_URL or NOTIFY_SECRET not configured, skipping notifications')
    return
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.error('[SEJIN] Notify worker failed:', err)
    }

    return NextResponse.json({ success: true })
  } catch (error) {
    console.error('[SEJIN] Consult API error:', error)
    return NextResponse.json(
