...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('[SEJIN] Popups GET error:', error)
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('[SEJIN] Popups POST error:', error)
    return NextResponse.json(
      { success: false, error: (error as Error).message },
      { status: 500 }
    )
  ...
  try {
    ...
    if (updateData.이미지URL !== undefined) {
      try {
        ...
        console.error('[SEJIN] Failed to cleanup old popup image:', e)
      }
    }
    ...
    console.error('[SEJIN] Popups PUT error:', error)
    return NextResponse.json(
      { success: false, error: (error as Error).message },
      { status: 500 }
    )
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.error('[SEJIN] Failed to cleanup popup image on delete:', e)
    }
    ...
    console.error('[SEJIN] Popups DELETE error:', error)
    return NextResponse.json(
      { success: false, error: (error as Error).message },
      { status: 500 }
    )
